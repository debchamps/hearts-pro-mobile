...
export function HeartsGame({ initialPlayers, onExit, soundEnabled }: { initialPlayers: Player[], onExit: () => void, soundEnabled: boolean }) {
  ...
  const onDragEnd = (card: Card) => { if (dragInfo && dragInfo.id === card.id) { if (dragInfo.startY - dragInfo.currentY >= 50) handleHumanInteract(card); else handleHumanInteract(card); } setDragInfo(null); };
  ...
  return (
    <div className="h-screen w-full flex flex-col select-none relative overflow-hidden" onMouseMove={onDragMove} onTouchMove={onDragMove}>
      <div className="flex justify-between items-center px-4 pt-[var(--safe-top)] z-50 bg-black/80 h-16 shadow-2xl border-b border-white/5">
        ...
        <div className="text-center"><span className="text-[8px] text-white/40 font-black uppercase tracking-widest block leading-none mb-1">Round</span><span className="text-3xl font-black italic text-yellow-500">{gameState.roundNumber}</span></div>
        <div className="w-8" />
      </div>

      <div className="flex-1 relative">
        <Avatar player={gameState.players[2]} pos="top-4 left-1/2 -translate-x-1/2" active={gameState.turnIndex === 2} isWinner={clearingTrick?.winnerId === 2} gameType="HEARTS" phase={gameState.phase} />
